...
(function () {
  ...
  var Animation = function Animation(frame_set, delay) {
  ...
  Animation.prototype = {
    /* This changes the current animation frame set. For example, if the currentset is [0, 1], and the new set is [2, 3], it changes the set to [2, 3]. It alsosets the delay. */
    change: function change(frame_set) {
    ...
    update: function update() {
      ...
      if (this.count >= this.delay) {
        ...
        this.count = 0; // Reset the count.  /* If the frame index is on the last value in the frame set, reset to 0. If the frame index is not on the last value, just add 1 to it. */
  ...
  var buffer, controller, display, _loop, player, render, resize, sprite_sheet;
  ...
  controller = {
    ...
    keyUpDown: function keyUpDown(event) {
  ...
  player = {
    ...
    // You don't need to setup Animation right away.
    jumping: true,
    height: 16,
    width: 16,
    x: 0,
  ...
  sprite_sheet = {
    frame_sets: [[0, 1, 1, 1, 1], [2, 3], [4, 5]],
    // standing still, walk right, walk left
    image: new Image()
  };

  _loop = function loop(time_stamp) {
  ...
  sprite_sheet.image.addEventListener("load", function (event) {
    ...
    window.requestAnimationFrame(_loop); // Start the game loop.
  });
